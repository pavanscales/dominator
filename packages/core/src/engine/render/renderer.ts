...
// Try pool first — guard against pool overflow from concurrent pushes
        if (this._poolSize > 0 && this._poolSize <= this._nodePool.length) {
    ...
    clear(): void {
        ...
        for (let i = 0; i < this._nodesCap; i++) {
            ...
            if (node) {
                ...
                this._poolSize = this._nodePool.length;
                this._nodes[i] = null;
            }
        }
    }
