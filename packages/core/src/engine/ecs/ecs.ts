...
export function forEachDescendant(entityId: number, fn: (id: number, depth: number) => void): void {
    const w = getWorld();
    _dfsStackTop = 0;
    // Seed with the root; the root itself is never reported, only descendants.
    _dfsStack[_dfsStackTop++] = entityId;

    // Iterative DFS preorder in O(edges): push each node's children onto the
    // stack in reverse so they pop left-to-right, eliminating the O(n²) nested
    // sibling scans the previous implementation performed per node.
    const cap = _dfsStack.length;
    while (_dfsStackTop > 0) {
        const current = _dfsStack[--_dfsStackTop];
        if (current !== entityId) fn(current, w.depth[current]);

        let child = w.children[current];
        const base = _dfsStackTop;
        while (child >= 0) {
            if (_dfsStackTop < cap) _dfsStack[_dfsStackTop++] = child;
            child = w.nextSibling[child];
        }
        // Reverse the segment just pushed so children pop in forward preorder.
        let l = base;
        let r = _dfsStackTop - 1;
        while (l < r) {
            const t = _dfsStack[l];
            _dfsStack[l] = _dfsStack[r];
            _dfsStack[r] = t;
            l++;
            r--;
        }
